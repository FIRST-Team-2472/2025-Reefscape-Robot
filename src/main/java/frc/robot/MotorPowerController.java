...
public class MotorPowerController {
     ...
     * @param integralProportionalThreshold The error threshold in which the integral starts
     *     changing by smaller increments
     */
    public MotorPowerController(
        ...
        this.integralProportionalThreshold =
                integralProportionalThreshold * 2; // multiplies by 2 for math to work i
        // forgot why
        ...
        error = 0; // starts at 0
        timeIncrement =
                1 / time / 50; // 50 = code refresh rate (per second) this is the maximum amount the
        // integral
        // can
    ...
    public double calculate(double setpoint, double sensorRead) {
            ...
            if (error < 0 && integral > -1)
                integral -=
                        Math.min(
                                ...
                                timeIncrement
                                        * 2
                                        / (1 / Math.abs(error) * integralProportionalThreshold));
            else if (integral < 1)
                integral +=
                        Math.min(
                                timeIncrement,
        ...
        predictionPoint =
                (sensorRead - lastSensorRead) * dTime
                        + sensorRead; // predicts position dTime seconds in the
        // future
