...
public class Limelight {
  ...
  public PoseEstimate getTrustedPose() {
    ...
    if(poseA != null && isConnected(kFrontLimelightName)&&getClosestTagDist(kFrontLimelightName)<kMaxAllowedTagDistance) {poseATrust = isValid(kFrontLimelightName, poseA);}
    if(poseB != null && isConnected(kSideLimelightName)&&getClosestTagDist(kFrontLimelightName)<kMaxAllowedTagDistance) {poseBTrust = isValid(kSideLimelightName, poseB);}
    ...
    if(poseATrust) {limelightNames.add(kFrontLimelightName); }
    if(poseBTrust) {limelightNames.add(kSideLimelightName); }
  ...
  public void updateLoggingWithPoses() {
    if(isConnected(kSideLimelightName)) {
      if(LimelightHelpers.getBotPoseEstimate_wpiBlue_MegaTag2(kSideLimelightName) != null) {
        Pose2d poseB =
          LimelightHelpers.getBotPoseEstimate_wpiBlue_MegaTag2(kSideLimelightName).pose;
    ...
    if(isConnected(kFrontLimelightName)) {
      if(LimelightHelpers.getBotPoseEstimate_wpiBlue_MegaTag2(kFrontLimelightName) != null) {
        Pose2d poseA =
          LimelightHelpers.getBotPoseEstimate_wpiBlue_MegaTag2(kFrontLimelightName).pose;
   ...
   * 2+ tags or is closer then kMaxAllowedTagDistance (from constants) from the tag.
   *
