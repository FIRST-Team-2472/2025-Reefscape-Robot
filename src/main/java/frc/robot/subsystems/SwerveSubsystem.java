...
public class SwerveSubsystem extends SubsystemBase {
    ...
    private double frontLeftEncoderLast,
            frontRightEncoderLast,
            backLeftEncoderLast,
            backRightEncoderLast = 0;
    ...
    public MotorPowerController xPowerController,
            yPowerController,
            turningPowerController,
            speedPowerController;
    ...
    public SwerveSubsystem(PositionFilteringSubsystem positionFilteringSubsystem) {
        ...
        speedLimiter =
                new NewNewAccelLimiter(
                        TargetPosConstants.kForwardMaxAcceleration,
                        TargetPosConstants.kBackwardMaxAcceleration);

        // turningLimiter = new
        // NewAccelerationLimiter(TargetPosConstants.kForwardMaxAcceleration*2.5,
    ...
    public SwerveSubsystem(
            Pigeon2 gyro, // This constructor is used for testing
            SwerveModule frontLeft,
            SwerveModule frontRight,
            SwerveModule backLeft,
            SwerveModule backRight,
            GenericEntry headingShuffleBoard,
            GenericEntry odometerShuffleBoard,
            GenericEntry rollSB,
            GenericEntry pitchSB,
            PositionFilteringSubsystem positionFilteringSubsystem,
            int periods) {
        this.gyro = gyro;
    ...
    public Pose2d calculateFilteredPose(double odometryConfidence) {
        return this.positionFilteringSubsystem.getFilteredBotPose(
                getPose(),
                odometryConfidence); // This is RIGHT, it needs the filtered position back in
    }

    public Pose2d getPose() {
        Pose2d odometryPose = this.getOdometryPose();
        return new Pose2d(
                odometryPose.getX() + this.odometryOffset.getX(),
                odometryPose.getY() + this.odometryOffset.getY(),
    ...
    public void calibrateOdometry(double odometryConfidence) {
        ...
        this.odometryOffset =
                new Pose2d(
                        filteredPose.getX() - odometryPose.getX(),
                        filteredPose.getY() - odometryPose.getY(),
                        new Rotation2d());
    }
    ...
    public void initializeDriveToPointAndRotate(Pose2d targetPosition) {
        xError =
                targetPosition.getX()
                        - getPose().getX(); // getPose().getX() - targetPosition.getX();
        yError =
                targetPosition.getY()
                        - getPose().getY(); // getPose().getY() - targetPosition.getY();
        ...
        Rotation2d angleDifference =
                odometer.getPoseMeters().getRotation().minus(targetPosition.getRotation());
    ...
    public void executeDriveToPointAndRotate(Pose2d targetPosition) {
        ...
        xSpeed =
                xError
                        / (Math.abs(xError) > Math.abs(yError)
                                ? Math.abs(xError)
                                : Math.abs(yError));
        ySpeed =
                yError
    ...
    public void runModulesRobotRelative(ChassisSpeeds chassisSpeeds) {
        ...
        chassisSpeeds = ChassisSpeeds.fromRobotRelativeSpeeds(chassisSpeeds, getRotation2d());
    ...
    public boolean isExactlyInPosition(Pose2d targetPosition) {
        return isAtPoint(targetPosition.getTranslation())
                && isAtAngle(targetPosition.getRotation());
    }

    public boolean isNearlyInPosition(Pose2d targetPosition) {
        return isNearPoint(targetPosition.getTranslation())
                && isNearAngle(targetPosition.getRotation());
    }

    public boolean isAtPoint(Translation2d targetDrivePos) {
        SmartDashboard.putNumber(
                "translation Error", getPose().getTranslation().getDistance(targetDrivePos));
        boolean isAtPose =
                getPose().getTranslation().getDistance(targetDrivePos)
    ...
    public boolean isNearPoint(Translation2d targetDrivePos) {
        ...
        boolean isNearPose =
                getPose().getTranslation().getDistance(targetDrivePos)
                        <= TargetPosConstants.kAcceptableDistanceError * 2.5; //
    ...
    public void periodic() {
        ...
        LimelightHelpers.SetRobotOrientation(
                SensorConstants.PRIMARY_LIMELIGHT,
        ...
        SmartDashboard.putNumber(
                "Frontleft Encoder rate of change",
                frontLeft.getDrivePositionTwo() - frontLeftEncoderLast);
        SmartDashboard.putNumber(
                "FrontRight Encoder rate of change",
                frontRight.getDrivePositionTwo() - frontRightEncoderLast);
        SmartDashboard.putNumber(
                "BackLeft Encoder rate of change",
                backLeft.getDrivePositionTwo() - backLeftEncoderLast);
        SmartDashboard.putNumber(
                "BackRight Encoder rate of change",
                backRight.getDrivePositionTwo() - backRightEncoderLast);
        ...
        SmartDashboard.putNumber(
                "BackLeft Encoder",
                backLeft.absoluteEncoder.getAbsolutePosition().getValueAsDouble());
        SmartDashboard.putNumber(
                "BackRight Encoder",
        ...
        SmartDashboard.putNumber(
                "odometerAngle", odometer.getPoseMeters().getRotation().getDegrees());
