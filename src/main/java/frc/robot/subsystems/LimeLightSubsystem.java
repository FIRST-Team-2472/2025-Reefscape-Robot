	...
	public void fetchLimeLightData() {
		...
		for (int i = 0; i < LimeLights.length; i++) {
			...
			double confidence = Math.min(
					((32.0f / (targetDistance + 4.0f)) - (0.25f * targetDistance) + (7.0f * targetArea)) * 0.1f + 0.02f,
					1.0f);
			...
			System.out.println("LimeLightSubsystem: Name: " + LimeLights[i] + ", Confidence: " + confidence
					+ ", Pose: (" + estimate.pose.getX() + ", " + estimate.pose.getY() + ")");
		}
	}
