...
public class ElevatorSubsystem extends SubsystemBase {
  ...
  public ElevatorSubsystem() {
    ...
    config.idleMode(
        IdleMode.kCoast); // they didnt move on brake mode, we also dont really need it on that mode
    config.apply(limitSwitchConfig);
    rightElevatorMotor.configure(
        config, ResetMode.kResetSafeParameters, PersistMode.kPersistParameters);
    config.follow(
        rightElevatorMotor); // change the config to copy the right motor before assigning it to the
    // left
  ...
  public void periodic() {
    // run the elevator to the correct height this is in the subsystem so it doesnt need a command
    // to hold its height
