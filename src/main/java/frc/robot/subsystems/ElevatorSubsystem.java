...
public class ElevatorSubsystem extends SubsystemBase {
    ...
    private SparkMax leftElevatorMotor =
            new SparkMax(ElevatorConstants.kLeftElevatorMotorID, MotorType.kBrushless);
    private SparkMax rightElevatorMotor =
            new SparkMax(ElevatorConstants.kRightElevatorMotorID, MotorType.kBrushless);
    ...
    public ElevatorSubsystem() {
        ...
        config.idleMode(
                IdleMode.kCoast); // they didnt move on brake mode, we also dont really need it on
        // that mode
        config.apply(limitSwitchConfig);
        rightElevatorMotor.configure(
                config, ResetMode.kResetSafeParameters, PersistMode.kPersistParameters);
        config.follow(
                rightElevatorMotor); // change the config to copy the right motor before assigning
        // it to the left
    ...
    public void periodic() {
        // run the elevator to the correct height this is in the subsystem so it doesnt need a
        // command to hold its height

        // updating the sensors status to be read by other files
        // code to check that both motors are working and returning the other motors value if one
        // isnt
        if (!leftElevatorMotor.hasActiveFault()
                && leftElevatorMotor.getEncoder().getPosition() != lastLeftElevatorReading) {
            RobotStatus.kElevatorHeight =
                    leftElevatorMotor.getEncoder().getPosition()
                            * ElevatorConstants.kElevatorMotorRotationsToInches;
        } else {
            RobotStatus.kElevatorHeight =
                    rightElevatorMotor.getEncoder().getPosition()
                            * ElevatorConstants.kElevatorMotorRotationsToInches;
        }
