...
public class NewAccelerationLimiter {
	private double positiveRateLimit, negativeRateLimit, previousSpeed, previousTime, elapsedTime, currentTime,
			wantedAcceleration, clampedAcceleration, clampedSpeed;
	...
	public double calculate(double MPCwantedSpeed) {
		...
		clampedAcceleration = MathUtil.clamp(wantedAcceleration, negativeRateLimit * elapsedTime,
				positiveRateLimit * elapsedTime); // Clamped change in speed based on time elapsed
		clampedSpeed = MathUtil.clamp(previousSpeed + clampedAcceleration, -1, 1); // changes motor speed to new clamped
