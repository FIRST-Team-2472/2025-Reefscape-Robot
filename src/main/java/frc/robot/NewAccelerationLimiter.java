...
public class NewAccelerationLimiter {
    ...
    public double calculate(double MPCwantedSpeed) {
        ...
        wantedAcceleration =
                Math.abs(MPCwantedSpeed)
                        - previousSpeed; // Change in speed wanted by the Motor Power Controller
        ...
        clampedAcceleration =
                MathUtil.clamp(
                        ...
                        positiveRateLimit
                                * elapsedTime); // Clamped change in speed based on time elapsed
        clampedSpeed =
                MathUtil.clamp(
                        previousSpeed + clampedAcceleration + lastAcceleration,
                        -1,
                        1); // changes motor speed to new clamped speed based on acceleration limits
