...
public class SwerveFollowTransitionCmd extends Command {
  ...
  public SwerveFollowTransitionCmd(
    ...
    angleTransitionPerFrame =
        endPose.getRotation().minus(startPose.getRotation()).getDegrees() / 50 / transitionTime;
  ...
  public boolean isFinished() {
    ...
    if (swerveSubsystem.getChassisSpeedsRobotRelative().vxMetersPerSecond < 0.05
        && swerveSubsystem.getChassisSpeedsRobotRelative().vyMetersPerSecond < 0.05) return true;

    if (timer.hasElapsed(3)) return true;
